...
impl LayerWeightsNoCache {
    fn mask(
        ...
        let mask: Vec<_> = (0..seq_len)
            .flat_map(|i| {
                (0..seq_len + index_pos).map(move |j| {
                    if j > i + index_pos {
                        f32::NEG_INFINITY
                    } else {
                        0.0
            ...
            .collect();
        let mask = Tensor::from_slice(&mask, (seq_len, seq_len + index_pos), device)?;

        // Apply sliding window if needed
        let mask = if let Some(window_size) = self.sliding_window_size {
            let sliding_mask: Vec<_> = (0..seq_len)
                .flat_map(|i| {
                    (0..seq_len + index_pos).map(move |j| {
                        if j > i + index_pos || (j + window_size) < (i + index_pos) {
                            f32::NEG_INFINITY
                        } else {
                            0.0
                        }
                ...
                .collect();
            let sliding_mask =
                Tensor::from_slice(&sliding_mask, (seq_len, seq_len + index_pos), device)?;
            mask.maximum(&sliding_mask)?
        } else {
            mask
        };

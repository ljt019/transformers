...
fn parse_tool_config(args: TokenStream) -> (proc_macro2::TokenStream, proc_macro2::TokenStream) {
    let default_error_strategy = quote! { transformers::pipelines::text_generation::text_generation_model::ErrorStrategy::Fail };
...
fn parse_error_strategy_from_expr(expr: &syn::Expr) -> proc_macro2::TokenStream {
    ...
    if expr_str == "Fail" || expr_str.contains("ErrorStrategy::Fail") {
        quote! { transformers::pipelines::text_generation::text_generation_model::ErrorStrategy::Fail }
    } else if expr_str == "ReturnToModel" || expr_str.contains("ErrorStrategy::ReturnToModel") {
        quote! { transformers::pipelines::text_generation::text_generation_model::ErrorStrategy::ReturnToModel }
    } else {
...
pub fn tool(args: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let wrapper_body = if is_result {
        quote! {
            #( #extraction_stmts )*
            use transformers::pipelines::text_generation::tool_error::ToolError;
    ...
    let expanded = quote! {
        ...
        #[doc(hidden)]
        fn #wrapper_name(mut parameters: std::collections::HashMap<String, String>) -> Result<String, transformers::pipelines::text_generation::tool_error::ToolError> {
            #wrapper_body
        }

        // Hidden function used by the tools! macro
        #[doc(hidden)]
        pub fn #tool_builder_name() -> transformers::pipelines::text_generation::text_generation_model::Tool {
            let mut parameters = std::collections::HashMap::new();
            #( #schema_kv_pairs )*

            transformers::pipelines::text_generation::text_generation_model::Tool::new(
        ...
        pub mod #fn_name_ident {
            ...
            pub fn __tool() -> transformers::pipelines::text_generation::text_generation_model::Tool {
                #tool_builder_name()
            }
        }
    };
